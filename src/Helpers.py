...
MOVIE_COLUMNS_TO_CLEAN = [
    "Fb_movie_id",
    "Movie languages",
    "Movie genres",
    "Movie countries",
]
CHARACTER_COLUMNS_TO_CLEAN = [
    "Fb_movie_id",
    "Fb_actor_ethnicity_id",
    "Fb_char_id",
    "Fb_actor_id",
...
def load_movie_metadata():
    ...
    df = pd.read_csv(
        DATA_FOLDER + "movie.metadata.tsv",
        sep="\t",
        header=None,
        names=[
            "Wiki_movie_id",
    ...
    df["release_date"] = pd.to_datetime(
        df["release_date"], format="mixed", utc=True, errors="coerce"
    ).dt.year

    # Remove "language" in the 'Movie languages' column
    df["Movie languages"] = df["Movie languages"].str.replace(
        "language", "", case=False
    )

    # Handle extreme values in 'Movie runtime'
    df["Movie runtime"] = df["Movie runtime"].apply(
...
def load_plot_summaries():
    # Read the csv file as a pandas dataframe
    df = pd.read_csv(
        DATA_FOLDER + "plot_summaries.txt",
        sep="\t",
        header=None,
...
def load_character_metadata():
    # Read the csv file as a pandas dataframe
    df = pd.read_csv(
        DATA_FOLDER + "character.metadata.tsv",
        sep="\t",
        header=None,
        names=[
            "Wiki_movie_id",
...
def load_name_clusters():
    # Read the csv file as a pandas dataframe
    df = pd.read_csv(
        DATA_FOLDER + "name.clusters.txt",
        sep="\t",
        header=None,
        names=["Name", "Fb_char_actor_id"],
    )
...
def load_tvtropes_clusters():
    # Read the csv file as a pandas dataframe
    df = pd.read_csv(
        DATA_FOLDER + "tvtropes.clusters.txt",
        delimiter="\t",
        header=None,
        names=["CharType", "Values"],
    )
